...
function recycledPhrasingIssues(articles: readonly Article[], file: string): CheckIssue[] {
...
export function checkArticles(input: ArticleCheckInput): ArticleCheckResult {
  ...
  // Rule 15 is the one rule no single article can break, so it runs once the loop has
  // every article rather than inside it.
  issues.push(...recycledPhrasingIssues(input.articles, file));
