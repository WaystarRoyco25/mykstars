...
import { ARTICLE_WINDOW_DAYS, DAY_MS } from "../policy/time";
import { issue, type CheckIssue } from "./result";

export const ANALYSIS_BYLINE = "MyKStars";

export interface ArticleStructureInput {
...
export function checkArticleStructure(
  ...
  for (const [index, article] of input.articles.entries()) {
    ...
    if (!article.slug) {
      issues.push(
        issue(file, "malformed article literal", `article #${index + 1}: missing slug`),
      );
    ...
    if (!article.date) {
      issues.push(
        issue(
          ...
          `${label}: every article carries its real publish date`,
        ),
      );
    } else {
